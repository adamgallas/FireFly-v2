...
class Conv2dv2[T <: Data](dataType: HardType[T], pp: Int, depth: Int) extends Component {
  ...
  val config = new Bundle {
    val padCtrl = new Bundle {
      val channels = UInt(boundBitWidth.head bits)
      val imgWidth = UInt(boundBitWidth.takeRight(2).head bits)
      val imgHeight = UInt(boundBitWidth.takeRight(2).last bits)
      val imgLeftPad = UInt(16 bits)
      val imgUpPad = UInt(16 bits)
      val imgRightPad = UInt(16 bits)
      val imgDownPad = UInt(16 bits)
    }

    val pushCtrl = new Bundle {
      val bndOfChannels = UInt(10 bits)
      val bndOfWidth = UInt(10 bits)
      val incOfnextLine = UInt(width bits)
    }

    val cacheCtrl = new Bundle {
      val cacheSize = UInt(width bits)
      val cacheDummy = UInt(width bits)
    }

    val convCtrl = new Bundle {
      val length = UInt(2 bits)
      val dilation = UInt(2 bits)
      val incOfNextImg = UInt(width bits)
      val bndOfLoop = Vec(boundBitWidth.map(w => UInt(w bits)))
      val incOfAddrLv1 = Vec(UInt(width bits), boundBitWidth.length)
      val incOfAddrLv2 = Vec(UInt(width bits), 2)
    }

    val ctrlBit = in Vec(
      Bits(padCtrl.getBitsWidth bits),
      Bits(pushCtrl.getBitsWidth bits),
      Bits(cacheCtrl.getBitsWidth bits),
      Bits(convCtrl.getBitsWidth bits)
    )

    padCtrl.assignFromBits(ctrlBit(0))
    pushCtrl.assignFromBits(ctrlBit(1))
    cacheCtrl.assignFromBits(ctrlBit(2))
    convCtrl.assignFromBits(ctrlBit(3))
