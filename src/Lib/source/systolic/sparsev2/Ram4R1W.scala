...
class Ram4R1W[T <: Data](dataType: HardType[T], depth: Int) extends Component {
  val addressWidth = log2Up(depth)

  val io = new Bundle {
    val write = slave(Flow(Fragment(Vec(dataType, 2))))
  ...
  val writeXor = io.write.fragment(0).asBits ^ io.write.fragment(1).asBits
  when(io.write.valid) {
    writeCnt := writeCnt + 1
    when(io.write.last) {
      writeCnt.clearAll()
  ...
  val readOnly = new Area {
    ...
    val xorOut = xorRam.readSync(xorAddrR)
    val ram0Out = ram(0).readSync(ram0AddrR)
    val ram1Out = ram(1).readSync(ram1AddrR)
  ...
  val readWrite = new Area {
    ...
    val xorAddr = Mux(wen, writeCnt, xorAddrR)
    val ram0Addr = Mux(wen, writeCnt, ram0AddrR)
    val ram1Addr = Mux(wen, writeCnt, ram1AddrR)

    val xorOut = xorRam.readWriteSync(xorAddr, writeXor, True, wen)
...
object Ram4R1W extends App {
  SpinalConfig(
    defaultConfigForClockDomains = ClockDomainConfig(resetKind = SYNC, resetActiveLevel = HIGH),
    targetDirectory = "data//verilog//Lib",
    anonymSignalPrefix = "tmp"
  ).generateVerilog(
    new Ram4R1W(Bits(32 bits), 1024)
  )
}
