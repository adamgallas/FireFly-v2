...
case class Int8MsVsNsTAcc(parallelism: (Int, Int, Int, Int)) extends Component {
  ...
  val io = new Bundle {
    ...
    val outputs = master(Flow(Fragment(Vec(Vec(Vec(Bits(18 bits), 4), t), m))))
  }
  noIoPrefix()

  val init = SystolicInit(Vec(Bits(t bits), 2), Vec(Vec(Bits(8 bits), 4), 2), (m, v, n))
  val chain = Array.fill(n)(Array.fill(m)(Int8VsVsT(t, v)))
  val gather = Array.fill(m)(new LadderFlowGather(Vec(Vec(Bits(18 bits), 4), t), n))
  val align = SystolicAlign(Vec(Vec(Bits(18 bits), 4), t), m)
...
object Int8MsVsNsTAcc extends App {
  SpinalVerilog(new Int8MsVsNsTAcc((4, 4, 8, 4)))
}