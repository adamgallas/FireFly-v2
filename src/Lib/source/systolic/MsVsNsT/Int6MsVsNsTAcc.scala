...
case class Int6MsVsNsTAcc(parallelism: (Int, Int, Int, Int)) extends Component {
  val (m, v, n, t) = parallelism
  val io = new Bundle {
    val inputs = slave(Flow(Fragment(utils.Linked(Vec(Vec(Vec(Bits(t bits), 2), v), n), Vec(Vec(Vec(Vec(Bits(6 bits), 4), 2), v), m)))))
    val outputs = master(Flow(Fragment(Vec(Vec(Vec(Bits(12 bits), 4), t), m))))
  }
  noIoPrefix()

  val init = SystolicInit(Vec(Bits(t bits), 2), Vec(Vec(Bits(6 bits), 4), 2), (m, v, n))
  val chain = Array.fill(n)(Array.fill(m)(Int6VsVsT(t, v)))
...
object Int6MsVsNsTAcc extends App {
