...
class LIF4StepsDynamicPostProcess(width: Int) extends Component {
  ...
  val PAt = (prev >> 1) > io.subRes(0)
  val PABt = (prev >> 2) > io.subRes(1)
  val PABCt = (prev >> 3) > io.subRes(2)
  val PABCDt = (prev >> 4) > io.subRes(3)
  ...
  val PABCD = (ABCD.expand + (prev >> 4)).sat(1)
...
object LIF4StepsDynamicPostProcess extends App {
  SpinalVerilog(new LIF4StepsDynamicPostProcess(16))
}